...
# this model generate tokens sequentially, and work with vqvae v21
# which encode trajectories without overlay
# from v2, only that at each execution, we rollout the whole trajectory
...
class PolicyTransformer(pl.LightningModule):
    ...
    def rollout_gt(self, traj_code: torch.Tensor, rgb: torch.Tensor, pcd: torch.Tensor,
        ...
        for i in tqdm(range(traj_code.shape[1]), desc="Rollout Ground Truth"):
